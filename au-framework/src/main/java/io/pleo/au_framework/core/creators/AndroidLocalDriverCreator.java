...
/** A concrete class that can create local Android WebDrivers only. */
public class AndroidLocalDriverCreator extends BaseDriverCreator {

  private static final Logger LOGGER = LogManager.getLogger(AndroidLocalDriverCreator.class);
  ...
  public WebDriver createDriver(JsonConfig jsonConfig) {
    ...
    if (jsonConfig.getExecutionMode().equals(ExecutionMode.LOCAL)
      ...
      LOGGER.debug(
          String.format(
              Constants.WEBDRIVER_CREATED_SUCCESSFULLY_MESSAGE,
              Platform.ANDROID,
              ExecutionMode.LOCAL));
      logSessionID(driver);
      return driver;
